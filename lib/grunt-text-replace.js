...
gruntTextReplace = {
  ...
  replaceFile: function (settings) {
    ...
    var replacements = settings.replacements;
    var inplace = pathToSourceFile===pathToDestinationFile;
    var needsMove = false
    var pathToDestinationFilePotentialTmp = pathToDestinationFile + (inplace? ".tmp" : "" );
    grunt.file.copy(pathToSourceFile, pathToDestinationFilePotentialTmp, {
      process: function (text) {
        processed = gruntTextReplace.replaceTextMultiple(text, replacements);
        needsMove = inplace && processed !== text
        return processed
      }
    });
    if (needsMove) {
      grunt.file.delete(pathToDestinationFile);
      grunt.file.copy(pathToDestinationFilePotentialTmp, pathToDestinationFile);
    }
    if (inplace) {
      grunt.file.delete(pathToDestinationFilePotentialTmp);
    }
  },
